...
class DPOConfig:
    ...
    # χPO (Chi-squared Preference Optimization) parameters
    xpo: bool = False
    """Whether to use χPO loss (Huang et al., 2024). Replaces DPO's log(π/π_ref) link
    with ϕ(z) = z + log(z) to induce mixed χ²+KL regularization."""
    xpo_r_max: float = 1.0
    """R_max for the implicit-reward clipping range [-2·R_max, +2·R_max] (Eq. 9 of χPO paper)."""

    # AuxDPO parameters
    auxdpo: bool = False
    """Whether to use AuxDPO (auxiliary variable DPO) to mitigate reward misspecification"""
    auxdpo_lambda_null: float = 1.0
    """Penalty weight for the null-space constraint ‖A_θ₀ · δ‖²"""
    auxdpo_lambda_amp: float = 0.01
    """Small L2 regularizer on δ to avoid trivial collapse"""
    auxdpo_delta_cap: float = 1.0
    """Clamp |δ| via tanh to this value for stability"""
    auxdpo_aux_lr: float = 5e-3
    """Learning rate for the auxiliary δ parameters (separate from model lr)"""
    ...
    """Comma-separated list of module names to apply LoRA to (e.g. 'q_proj,v_proj'). If None, uses default for model arch."""
